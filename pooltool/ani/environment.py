...
class Environment:
    ...
    def init(self, table):
        if settings.graphics.physical_based_rendering:
            room_model_path = model_dir / "room/room_pbr.glb"
            floor_model_path = model_dir / "room/floor_pbr.glb"
        else:
            room_model_path = model_dir / "room/room.glb"
            floor_model_path = model_dir / "room/floor.glb"

        room_path = panda_path(room_model_path)
        floor_path = panda_path(floor_model_path)

        self.set_table_offset(table)

        if settings.graphics.room and room_model_path.exists():
